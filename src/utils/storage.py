    ...
    def __init__(self):
        ...
        if os.environ.get("BUCKETEER_BUCKET_NAME"):
            ...
            try:
                ...
                logger.info(
                    f"Bucketeer storage initialized with bucket: {self.s3_bucket}"
                )
            ...
            try:
                if binary:
                    self.s3_client.put_object(
                        Bucket=self.s3_bucket, Key=s3_key, Body=content
                    )
                else:
                    self.s3_client.put_object(
    ...
    def read_file(self, file_path, binary=False):
        ...
        if file_path.startswith("s3://"):
            if not self.s3_enabled:
                raise ValueError(
                    "S3 storage is not configured but an S3 URL was provided"
                )
        ...
        if self.s3_enabled:
            ...
            try:
                ...
                if "Contents" in response:
                    return [
                        obj["Key"][len(s3_prefix) :] for obj in response["Contents"]
                    ]
                return []
            except ClientError as e:
