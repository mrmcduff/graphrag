...
def get_non_interactive_llm_config(provider_id: int, provider_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    ...
    if provider_id == 1:  # Local API
        ...
        if 'api_key' not in config:
            ...
            config['api_key'] = api_key if api_key else ''
        if 'model' not in config:
            config['model'] = os.environ.get('ANTHROPIC_MODEL', 'claude-3-haiku-20240307')
