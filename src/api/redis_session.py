...
def get_redis_client() -> redis.Redis:
    ...
    if _redis_client is None:
        ...
        if not redis_url:
            logger.warning(
                "REDIS_URL not found in environment, using in-memory storage"
            )
            return None
...
def serialize_game_state(session: GameSession) -> Dict[str, Any]:
    ...
    visited_locations = (
        list(game_state.visited_locations)
...
def load_session(session_id: str) -> Optional[GameSession]:
    ...
    try:
        ...
        if game_state_data:
            session.game_state.player_location = game_state_data.get(
                "player_location", "start"
            )
            session.game_state.inventory = game_state_data.get("inventory", [])
            session.game_state.visited_locations = set(
                game_state_data.get("visited_locations", [])
            )
