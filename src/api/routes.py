...
def new_game():
    ...
    if not isinstance(provider_id, int) or provider_id < 1 or provider_id > 6:
        return jsonify(format_error_response("Invalid provider_id. Must be an integer between 1 and 6.", 400)), 400
        
    # Validate provider_config
    if not isinstance(provider_config, dict):
        return jsonify(format_error_response("provider_config must be a dictionary.", 400)), 400
    ...
    try:
        ...
        return jsonify(format_error_response(f"Error creating game session: {str(e)}")), 500
...
def process_command(session_id):
    ...
    try:
        ...
        return jsonify(format_error_response(f"Error processing command: {str(e)}")), 500
    ...
    try:
        ...
        if success:
            ...
            return jsonify({
                ...
                "player_location": game_sessions[session_id].game_state.player_location,
...
def get_game_state(session_id):
    ...
    try:
        ...
        return jsonify(format_error_response(f"Error getting game state: {str(e)}", 500)), 500
...
def set_llm_provider(session_id):
    ...
    try:
        ...
        return jsonify(format_error_response(f"Error setting LLM provider: {str(e)}", 500)), 500
...
def end_game_session(session_id):
    ...
    try:
        ...
        return jsonify(format_error_response(f"Error ending game session: {str(e)}", 500)), 500
