...
def list_document_folders():
    ...
    """
    try:
        base_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "data", "documents")
        ...
        for item in os.listdir(base_dir):
            ...
            if os.path.isdir(item_path):
                folders.append({
                    ...
                    "file_count": len([f for f in os.listdir(item_path) if f.endswith('.docx')])
                })
        ...
        return jsonify(format_error_response(f"Error listing document folders: {str(e)}", 500)), 500
...
def list_available_worlds():
    ...
    """
    try:
        base_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "data", "output")
        ...
        if os.path.exists(os.path.join(base_dir, "knowledge_graph.gexf")):
            worlds.append({
                "name": "default",
                "path": "data/output",
                "created": time.ctime(os.path.getmtime(os.path.join(base_dir, "knowledge_graph.gexf"))),
        ...
        return jsonify(format_error_response(f"Error listing available worlds: {str(e)}", 500)), 500
...
def create_new_world():
    ...
    output_name = data.get("output_name")  # Will default to input directory name in document_processor
    ...
    if not documents_dir:
        return jsonify(format_error_response("No documents directory specified", 400)), 400
    ...
    try:
        ...
        return jsonify({
            ...
            "world": {
                ...
                "path": rel_world_dir,
                "created": time.ctime(os.path.getmtime(os.path.join(world_dir, "knowledge_graph.gexf"))),
                "entities_count": count_entities(world_dir)
            }
        })
    except Exception as e:
        return jsonify(format_error_response(f"Error creating world: {str(e)}", 500)), 500
...
def new_game_from_world():
    ...
    try:
        ...
        if not os.path.exists(os.path.join(abs_world_path, "knowledge_graph.gexf")):
            return jsonify(format_error_response(f"World not found at {world_path}", 404)), 404
