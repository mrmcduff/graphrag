...
def parse_arguments():
    ...
    parser.add_argument("--game_data_dir", default="data/output",
                      help="Directory containing game data files")
    parser.add_argument("--load_save", default=None,
                      help="Load a previously saved game file")
    parser.add_argument("--no_color", action="store_true",
                      help="Disable colored output")
    parser.add_argument("--no_typing_effect", action="store_true",
                      help="Disable typing effect for text display")
    parser.add_argument("--width", type=int, default=80,
                      help="Width of the text display")
...
def validate_data_directory(game_data_dir: str) -> bool:
    ...
    missing_files = [f for f in required_files
                    if not os.path.exists(os.path.join(game_data_dir, f))]

    if missing_files:
        print(f"Warning: The following required files are missing from '{game_data_dir}':")
        for file in missing_files:
            print(f"  - {file}")

        print("\nYou might need to run the document processor first:")
        print("  python src/document_processor.py --documents_dir data/documents --output_dir data/output")
